...
type Status struct {
	...
	// The status codes, which should be an enum value of [codes.Code][codes.Code].
...
// GetCode returns the status codes contained in s.
...
func (s *Status) String() string {
	return fmt.Sprintf("codes = %d desc = %s", s.GetCode(), s.GetMessage())
}
